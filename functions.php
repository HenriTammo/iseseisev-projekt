...
function signin($email, $password){
	...
	$stmt = $mysqli->prepare("SELECT id, username, password FROM userForInsta WHERE email=?");
	echo $mysqli->error;
...
function signup($username, $email, $password){
	...
	$stmt = $mysqli->prepare("SELECT id FROM userForInsta WHERE email=?");
	echo $mysqli->error;
	$stmt->bind_param("s",$email);
	$stmt->execute();
	if($stmt->fetch()){
		...
		$stmt->close();
		$stmt = $mysqli->prepare("INSERT INTO userForInsta (username, email, password) VALUES(?,?,?)");
    	echo $mysqli->error;
